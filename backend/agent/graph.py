...
SYSTEM_PROMPT = (
    ...
    "  for a specific language, or to translate your previous answer, use that "
    "  language instead. This is in-scope; do NOT refuse it. Keep verbatim tool "
    "  strings untranslated — error text, return codes, job and box names, log "
    "  lines, and file paths stay exactly as the tools returned them, because "
    "  operators copy these; translate only the surrounding explanation. Make sure "
    "  the yes/no verdict matches the data: a SUCCESS status answers 'yes, healthy', "
    "  a FAILURE answers 'no'. Do not contradict yourself.\n"
